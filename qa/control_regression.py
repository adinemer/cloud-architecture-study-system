...
def session_fixture():
    ...
    return {
      'session_id':sid,'schema_version':'1.2.0','certification':'SAP-C02','exam_scope_version':PROJECT['exam_scope_version'],
      'unit_ids':['U00'],'objective_ids':[],'session_type':'SCOPE_BASELINE','status':'PLANNED','control_snapshot':build_snapshot(sid),
      'authorities_consulted':sorted(REQUIRED_AUTHORITIES),'sources':[],'prerequisite_sessions':[],
      'predecessor_session_id':None,'consumed_handoff_id':None,'handoff_id':None,'required_artifact_types':[],
...
def main():
    s=session_fixture(); expect('current valid fixture passes', not errors(s))
    p=copy.deepcopy(PROJECT); p['study_status']='IN_PROGRESS'; p['active_session_id']=None; expect('in-progress requires active session', bool(validate_project(p)))
    m=copy.deepcopy(MASTERY); m['objectives']['1.1']['status']='COMPLETE'; m['objectives']['1.1']['E']=2; m['objectives']['1.1']['A']=3; expect('objective cannot complete below E3', any('E < 3' in x for x in validate_mastery(m)))
    s=session_fixture(); s['authorities_consulted'].remove('docs/12-coordinator-governance.md'); expect('missing authority fails closed', any('missing mandatory authorities' in x for x in errors(s)))
    s=session_fixture(); s['status']='PAUSED'; s['started_at']='2026-08-09T09:30:00Z'; s['pause_state']=None; expect('paused session requires resume state', any('PAUSED requires pause_state' in x for x in errors(s)))
    s=session_fixture(); s['status']='COMPLETED'; s['started_at']='2026-08-09T09:30:00Z'; s['ended_at']='2026-08-09T10:00:00Z'; s['completion_qa']='NOT_RUN'; s['next_permitted_action']='Next'; expect('completion requires QA pass', any('completion_qa=PASS' in x for x in errors(s)))
