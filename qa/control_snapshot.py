...
'docs/18-semantic-pipeline-integrity.md','docs/19-session-handoff-continuity.md','state/pipeline-health.json']
...
def build(session_id):
    ...
    if pipeline.get('health') != 'GREEN': raise RuntimeError(f"cannot create control snapshot while pipeline health is {pipeline.get('health')}")
    return {
      'snapshot_id':f"{session_id}-snapshot-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}",
      'repository_commit_sha':git_sha(),'created_at':datetime.now(timezone.utc).isoformat(),
      'governance_hashes':{x:h(x) for x in GOVERNANCE},'objective_map_hash':h('aws/sap-c02/objective-map.md'),
      'artifact_schema_version':'1.0.0','session_schema_version':'1.2.0',
      'prompt_versions':['extract-v1','enrich-v1','qa-v1','enrich-qa-v1','artifact-v1','artifact-qa-v1'],
      'project_state_version':project['state_version'],'mastery_state_version':mastery['state_version'],
      'pipeline_health_state_version':pipeline['state_version'],'pipeline_health':pipeline['health'],'pipeline_fingerprint':pipeline['pipeline_fingerprint']
    }

def main():
    p=argparse.ArgumentParser(); p.add_argument('--session-id',required=True); p.add_argument('--out'); a=p.parse_args(); d=build(a.session_id); text=json.dumps(d,indent=2)+'\n'
